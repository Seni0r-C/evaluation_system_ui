...
export const generarDocCalificacion = async (trabajo) => {
  try {
    // alert(JSON.stringify(trabajo, null, 2));
    // return { typeMsg: 'success', message: 'Acta generada exitosamente.'};
    const responseFileName = await axiosInstance.get(`notas/pdf-name/${trabajo.id}/${2}`);
    ...
    const responseDoc = await axiosInstance.get(`notas/pdf/${fileName}`, {
    ...
    a.click();
    return { typeMsg: 'success', message: 'Documento calificación generado exitosamente.'};
  } catch (error) {
    console.error('Error al generar el documento calificación en el cliente:', error);
    return { typeMsg: 'error', message: 'Error al generar el documento calificación en el cliente.' };
  }
};
