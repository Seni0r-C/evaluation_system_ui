...
export const UserProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    if (info) {
      ...
      setUser(parsedUser);
      setUserPhoto(`data:image/jpeg;base64,${parsedUser.fotoBase64}`);
      setRoles(parsedUser.roles || []);
      setUserName(getUserName(parsedUser));
      setRolesAsStr(getRolesText(parsedUser?.roles));
    }
    setLoading(false);
  }, []);
  ...
  return (
    <UserContext.Provider value={{userName, userPhoto, rolesAsStr, hasRole,  user, roles }}>
      {children}
